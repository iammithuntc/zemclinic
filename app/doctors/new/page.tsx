...
function NewDoctorForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.error || 'Failed to create doctor');
      }
    } catch (error) {
  ...
  return (
    <ProtectedRoute>
      <SidebarLayout 
        title={formData.role === 'admin' ? 'Add New Administrator' : formData.role === 'staff' ? 'Add New Staff Member' : 'Add New Doctor'}
        description={formData.role === 'admin' ? 'Create a new administrator account' : formData.role === 'staff' ? 'Create a new staff member account' : 'Create a new doctor or staff member account'}
      >
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-6">
            ...
            >
              ...
              <span>Back to {formData.role === 'admin' ? 'Administrators' : formData.role === 'staff' ? 'Staff' : 'Doctors'}</span>
            </Link>
          </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold text-white">
                    {formData.role === 'admin' ? 'Add New Administrator' : formData.role === 'staff' ? 'Add New Staff Member' : 'Add New Doctor'}
                  </h2>
                  <p className="text-sm text-white/80">
                    {formData.role === 'admin' ? 'Create a new administrator account' : formData.role === 'staff' ? 'Create a new staff member account' : 'Create a new doctor or staff member account'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    <p className="text-sm text-gray-700">
                      ...
                      {formData.role === 'doctor' && ' - Doctors can manage patients, appointments, and reports'}
                      {formData.role === 'staff' && ' - Staff members have the same access as doctors'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-6 border-t border-gray-200">
                ...
                >
                  ...
                  <span>{saving ? 'Creating...' : formData.role === 'admin' ? 'Create Administrator' : formData.role === 'staff' ? 'Create Staff Member' : 'Create Doctor'}</span>
                </button>
              </div>
            </form>
          </div>
