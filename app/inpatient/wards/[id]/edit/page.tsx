...
function EditWardForm({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchWard = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // API returns { ward, beds }, so extract ward from the response
        const ward = data.ward || data;
        setFormData({
          name: ward.name || '',
  ...
  if (!translationsLoaded) {
    return (
      <ProtectedRoute>
        <SidebarLayout title={t('inpatient.editWard')} description="">
  ...
  return (
    <ProtectedRoute>
      <SidebarLayout
        title={t('inpatient.editWard')}
        description={t('inpatient.editWardDescription')}
      >
...
export default function EditWardPage({ params }: { params: Promise<{ id: string }> }) {
  return (
    <Suspense fallback={
      <ProtectedRoute>
